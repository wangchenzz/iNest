...
@interface playViewController () <CircularProgressViewDelegate>
...
@property (nonatomic,strong)savevoicecontroller *url;
...
- (void)viewDidLoad
{
    ...
    self.circularProgressView.backColor = [UIColor colorWithRed:236.0 / 255.0 green:230 / 255.0 blue:230/ 255.0 alpha:1.0];
    self.circularProgressView.progressColor = [UIColor colorWithRed:82 / 255.0 green:135.0 / 255.0 blue:237.0 / 255.0 alpha:1.0];
    self.circularProgressView.audioPath = [[NSBundle mainBundle] pathForResource:self.codeUrl ofType:@"caf"];
...
//    }
    if (self.circularProgressView.audioPath == nil) {
        sender.enabled = NO;
    } else {
        sender.enabled = YES;
         if (sender.on) {
