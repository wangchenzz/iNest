...
-(void)textViewShowView:(UIViewController *)viewController customKeyboardDelegate:(id)delegate
{
    ...
    self.mBackView =[[UIView alloc]initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height-50-38, WIDTH_SCREEN, 50)];
...
{    
    self.isTop =NO;
        [UIView animateWithDuration:0.3 animations:^{
        self.mBackView.frame=CGRectMake(0, HEIGHT_SCREEN+40, WIDTH_SCREEN, 50);
        self.mHiddeView.alpha =0.0f;
...
- (void)textDidChanged:(NSNotification *)notif //监听文字改变 换行时要更改输入框的位置
{
    ...
    CGFloat selfHeight = self.mTextView.superview.frame.origin.y * 2 + contentSize.height - minus + 2 * 2+7;
    ...
    self.mSecondaryBackView.frame =CGRectMake(10, 10, 230, selfHeight-18);
   // NSLog(@"文字改变");
}
