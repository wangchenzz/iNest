...
- (void)viewDidLoad {
    ...
    self.menuDidClick(self.menuItems[0], YES,0);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return (CGRectGetHeight(self.tableView.bounds)-50)/ self.menuItems.count;
    //((CGRectGetHeight(self.tableView.bounds)-50) / self.menuItems.count)<80?((CGRectGetHeight(self.tableView.bounds)-50) / self.menuItems.count):80;
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.menuDidClick) {
    
        self.menuDidClick(self.menuItems[indexPath.row], NO,indexPath.row);
